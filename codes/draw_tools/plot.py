...
def plot(runner, axes, set=colors_set['Kraftime']):
    ...
    dev_iters = getattr(runner, "dev_iters", [i for i in range(len(runner.dev_scores))])
    # 绘制训练损失变化曲线
    ...
    # 绘制评价准确率变化曲线
    axes[1].plot(dev_iters, runner.dev_scores, color=dev_color, linestyle="--", label="Dev accuracy")
    # 绘制坐标轴和图例
