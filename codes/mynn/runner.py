    ...
    def train(self, train_set, dev_set, **kwargs):
        ...
        for epoch in range(num_epochs):
            ...
            for iteration in range(int(X.shape[0] / self.batch_size) + 1):
                ...
                should_evaluate = (iteration % log_iters == 0)
                if should_evaluate:
                    dev_score, dev_loss = self.evaluate(dev_set)
                    self.dev_scores.append(dev_score)
                    self.dev_loss.append(dev_loss)
                    self.dev_iters.append(len(self.train_scores) - 1)
            ...
            if last_dev_score > best_score:
                save_path = os.path.join(save_dir, 'best_model.pickle')
                self.save_model(save_path)
                print(f"best accuracy performence has been updated: {best_score:.5f} --> {last_dev_score:.5f}")
                best_score = last_dev_score
